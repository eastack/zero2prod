...
async fn spawn_app() -> TestApp {
    ...
    let listener = TcpListener::bind("127.0.0.1:0").expect("Successed to bind random port");
...
async fn health_check_works() {
    ...
    let response = client
        ...
        .send()
        .await
        .expect("Secceed to execute request.");
...
async fn subscribe_returns_a_200_for_valid_form_date() {
    ...
    let response = client
        ...
        .send()
        .await
        .expect("Subscriptions api call successed");
    ...
    let saved = sqlx::query!("select email, name from subscriptions")
        ...
        .await
        .expect("Fetch saved subscription succeed.");
